            ...
            {chatHistory.map((chat, idx) => (
              ...
              >
                <div className={`flex gap-2 max-w-[85%] ${chat.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                  <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${chat.role === 'user' ? 'bg-slate-700' : 'bg-blue-600/20 border border-blue-500/30'
                    }`}>
                    {chat.role === 'user' ? <User className="w-4 h-4 text-white" /> : <Bot className="w-4 h-4 text-blue-400" />}
                  </div>
                  <div className={`p-3 rounded-2xl text-sm ${chat.role === 'user'
